...
pair<bitset <n>, bitset <n>> BCH_code_short_t3::divide_bitset_polynomials(
...
void begin_main_process(const int thread_id, const int thread_zone_beginning, const int thread_zone_end) {
	...
	for (int i = thread_zone_beginning; i < thread_zone_end; i++) {
		...
		BCH_objects[i] = make_shared<BCH_code_short_t3>(bitset<n>(vector_of_message_polynomials[i].to_string()));
...
int main(int argc, char* argv[]) {
	...
	string image_with_errors_path = filename.substr(0, filename.length()-4).append("_with_errors_BCH_code_short_t3.bmp");
	string image_fixed_path = filename.substr(0, filename.length()-4).append("_fixed_BCH_code_short_t3.bmp");
