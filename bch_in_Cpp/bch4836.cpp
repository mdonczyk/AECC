...
void begin_main_process(const int thread_id, const int thread_zone_beginning, const int thread_zone_end) {
	...
	for (int i = thread_zone_beginning; i < thread_zone_end; i++) {
		...
		BCH_objects[i] = make_unique<BCH_code_short_t2>(bitset<n>(vector_of_bitsets[i].to_string()), error_probability);
...
int main(int argc, const char* argv[]) {
	...
	string image_with_errors_path = string(argv[2]).substr(0, string(argv[2]).length()-4).append("_with_errors_BCH_code_short_t2.bmp");
	string image_fixed_path = string(argv[2]).substr(0, string(argv[2]).length()-4).append("_fixed_BCH_code_short_t2.bmp");
