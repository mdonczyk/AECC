...
void BCH_code_long_t2::encode_bch() {
...
std::vector <int> BCH_code_long_t2::calculate_syndromes(const std::bitset<n> &Received_Codeword, bool &errors_in_codeword) {
...
Status BCH_code_long_t2::decode_bch() {
	/*
...
std::pair<std::bitset <n>, std::bitset <n>> BCH_code_long_t2::divide_bitset_polynomials(
...
void BCH_code_long_t2::print_original_codeword_and_received_codeword() {
	if (verbose_flag) {
...
void BCH_code_long_t2::print_original_message_and_decoded_message() {
	if (verbose_flag) {
...
void BCH_code_long_t2::introduce_errors() {
...
void begin_main_process(const int thread_id, const int MBTG_beginning, const int MBTG_end) {
	...
	for (int i = MBTG_beginning; i < MBTG_end; i++) {
		...
		bch::BCH_objects[i] = make_shared<BCH_code_long_t2>(std::bitset<n>(bch::vector_of_message_polynomials[i].to_string()));
...
int main(int argc, char* argv[]) {
	...
	std::string image_with_errors_path = bch::filename.substr(0, bch::filename.length()-4).append("_with_errors_BCH_code_long_t2.bmp");
	std::string image_fixed_path = bch::filename.substr(0, bch::filename.length()-4).append("_fixed_BCH_code_long_t2.bmp");
	...
	//  	[](int sum, shared_ptr<BCH_code_long_t2> const& obj) 
