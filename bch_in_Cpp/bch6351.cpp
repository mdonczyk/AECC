...
vector <int> BCH_code_long_t2::calculate_syndromes(const bitset <n> &Received_Codeword, bool &syn_error) {
...
pair<bitset <k>, bool> BCH_code_long_t2::decode_bch(const bitset <n> &Received_Codeword) {
	/*
...
pair<bitset <n>, bitset <n>> BCH_code_long_t2::divide_bitset_polynomials(const bitset <n> &dividend, const bitset <n> &divisor) {
...
bitset <n> BCH_code_long_t2::multiply_bitset_polynomials(const bitset <n> &mulitplicand, const bitset <n> &multiplicator) {
	bitset <n> product;
	for (int i = 0; i < n; ++i) {
		if (multiplicator[i]) {
...
vector <bool> BCH_code_long_t2::bytes_to_bits (const vector <unsigned char> &buffer, const int fileSize) {
