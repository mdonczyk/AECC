...
vector <int> productPolynomial(50), errpos, zero_alphas;
...
class BCH_code{
	...
	public:
		...
		*/
			int i, j, feedback;
			for (i = 0; i < length - k; i++)  //length = 63, k = 51;
				rb[i] = 0;
...
-----------------------------------------------------------------------------------------------------------------
		index of alpha:					power of alpha								 |  MINIMAL POLYNOMIAL
		...
		*/
			...
			for (const auto& index : cycle_coset){
				...
				//populate zeros with cosets that have roots 1 to d-1
					zero_alphas.push_back(alpha_zero);
				}
				size = 0;
				if (roots_found == d - 1)
					break;
			...
			cout<<"g(x) = ";
			for (int i = rdncy; i>=0; i--) { // 1010100111001 == x^12 + x^10 + x^8 + 2x^7 + 2x^6 + x^5 + x^4 + x^3 + 2x^2 + 2x + 1
			cout<<g[i];
			}
			cout<<endl;
			auto stop = chrono::high_resolution_clock::now();
			auto duration = chrono::duration_cast<chrono::microseconds>(stop - start);
			cout <<"Count 1: "<< duration.count() << " microseconds" << endl;
			*/
		}

		void multiplyTwoPolynomials() {
			auto start = chrono::high_resolution_clock::now();
			int first_zero = 0, second_zero = 0, result = 0, temp = 0;
			int mask = 64;
			for (int i = 0; i < zero_alphas.size(); i++){
				first_zero = alpha_to[zero_alphas[i]];
				if (i + 1 == zero_alphas.size())
					break;
				else
					second_zero = alpha_to[zero_alphas[i + 1]];
				while (second_zero > 0 ){
					if (second_zero >> mask == 1){
						temp = first_zero << mask;
						result ^= temp;
						second_zero <<= 1;
						mask <<= 1;
					}
			...
			cout<<result<<endl; //should be 1010100111001
			auto stop = chrono::high_resolution_clock::now();
			auto duration = chrono::duration_cast<chrono::microseconds>(stop - start);
			cout <<"Count 2: "<< duration.count() << " microseconds" << endl;
		}
};
